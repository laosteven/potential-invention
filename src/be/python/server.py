...
computervision_client = ComputerVisionClient(
    COMPUTER_VISION_ENDPOINT, CognitiveServicesCredentials(COMPUTER_VISION_SUBSCRIPTION_KEY))
...
def send_plate_location():
    ...
    msg = bus_service.receive_subscription_message(
        'licenseplateread', 'eG4y7VYFse8NvW53', peek_lock=True)
...
def init_get_money():
    ...
    while True:
        msg = bus_service.receive_subscription_message(
            'licenseplateread', 'eG4y7VYFse8NvW53', peek_lock=False)
        ...
        if plate_num in plates:
            ...
            password = "RTFragcan38P5h8j"
            req_json = msg.body
            resp = requests.post(request_url, data=req_json,
    ...
    while True:
        ...
        if plate_num in plates:
            ...
            block_blob_service.create_blob_from_bytes(
                context_container_name, blob_name, base64.decodestring(img_context))
